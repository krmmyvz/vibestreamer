...
private:
    ...
    void insertToMemCache(const QString &url, const QPixmap &pixmap);

    QNetworkAccessManager m_net;
    QHash<QString, QPixmap> m_cache;
    QList<QString> m_lruOrder;        // LRU eviction order (front = oldest)
    ...
    static constexpr int MaxMemCache    = 500;  // max pixmaps kept in memory
};
